        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
            <nav class="flex" aria-label="Breadcrumb">
                <ol class="flex items-center space-x-2">
                    ...
                    <li>
                        <a href="{{ route('marketplace.index', ['category' => $product->category_id]) }}" class="text-gray-500 hover:text-gray-700">
                    ...
                    <div class="flex space-x-4 mt-4">
                        @foreach($product->images as $index => $image)
                            <button onclick="changeMainImage('{{ $image }}')"
                                    class="flex-shrink-0 w-20 h-20 rounded-lg overflow-hidden border-2 {{ $index == 0 ? 'border-blue-500' : 'border-gray-200' }} hover:border-blue-500 transition-colors">
            ...
            <div class="mt-10 px-4 sm:mt-16 sm:px-0 lg:mt-0">
                ...
                <div class="mt-4">
                    <div class="flex items-center">
                        ...
                        <span class="ml-2 text-sm text-gray-500">{{ $rating }} out of 5 stars</span>
                ...
                <div class="mt-6">
                    <div class="flex items-baseline">
                        ...
                        @if($product->sale_price && $product->sale_price < $product->price)
                            <span class="ml-3 text-xl text-gray-500 line-through">{{ $product->getFormattedPrice() }}</span>
                            <span class="ml-2 text-sm text-green-600 font-medium">Save {{ number_format((($product->price - $product->sale_price) / $product->price) * 100, 0) }}%</span>
                        @endif
                    </div>
                </div>
                ...
                <div class="mt-6">
                    @if($product->isInStock())
                        @if($product->isLowStock())
                            <p class="text-amber-600 font-medium">Only {{ $product->quantity }} left in stock!</p>
                        @else
                            <p class="text-green-600 font-medium">In stock and ready to ship</p>
                        @endif
                    @else
                        ...
                        <div class="mt-2 space-y-2">
                            @foreach($product->attributes as $key => $value)
                                <div class="flex justify-between py-2 border-b border-gray-200">
                                    <span class="text-sm font-medium text-gray-900">{{ ucfirst($key) }}</span>
                ...
                <div class="mt-10">
                        ...
                        <form id="addToCartForm" class="space-y-4">
                            ...
                            <div class="flex items-center space-x-4">
                                ...
                                <div class="flex items-center border border-gray-300 rounded-lg">
                                    <button type="button" onclick="decrementQuantity()" class="px-3 py-2 text-gray-600 hover:text-gray-900">
                                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M20 12H4"/>
                                        </svg>
                                    </button>
                                    <input type="number" id="quantity" name="quantity" value="1" min="1" max="{{ $product->quantity }}" class="w-16 text-center border-0 focus:ring-0">
                                    <button type="button" onclick="incrementQuantity()" class="px-3 py-2 text-gray-600 hover:text-gray-900">
                                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            ...
                            <!-- Add to Cart Button -->
                            <button type="submit" class="w-full bg-gradient-to-r from-blue-600 to-indigo-600 text-white px-8 py-4 rounded-lg font-semibold hover:from-blue-700 hover:to-indigo-700 transition-all duration-300 transform hover:scale-105 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2">
                    ...
                    @else
                        <button disabled class="w-full bg-gray-300 text-gray-500 px-8 py-4 rounded-lg font-semibold cursor-not-allowed">
                            Out of Stock
                        </button>
                    @endif
                </div>
                    ...
                    <div class="flex items-center justify-between">
                        <div class="flex items-center space-x-4">
                            ...
                            <div>
                                ...
                                <p class="text-sm text-gray-500">{{ $tenant->products()->where('is_public', true)->count() }} products</p>
                            </div>
                        </div>
                        <a href="{{ route('marketplace.index', ['team' => $tenant->id]) }}" class="text-blue-600 hover:text-blue-700 font-medium text-sm">
                            View Store →
                        </a>
                    </div>
                </div>
        ...
        <div class="mt-16">
            <div class="border-b border-gray-200">
                <nav class="-mb-px flex space-x-8">
                    <button onclick="showTab('details')" id="details-tab" class="tab-button border-b-2 border-blue-500 text-blue-600 py-2 px-1 text-sm font-medium">
                        Product Details
                    </button>
                    <button onclick="showTab('reviews')" id="reviews-tab" class="tab-button border-b-2 border-transparent text-gray-500 hover:text-gray-700 py-2 px-1 text-sm font-medium">
                        Reviews
                    </button>
                    <button onclick="showTab('shipping')" id="shipping-tab" class="tab-button border-b-2 border-transparent text-gray-500 hover:text-gray-700 py-2 px-1 text-sm font-medium">
                        Shipping & Returns
                    </button>
                </nav>
            </div>
...
<!-- Cart Toggle Button -->
<button onclick="window.location.href='{{ route('marketplace.index') }}'" class="fixed bottom-6 right-6 bg-blue-600 text-white p-4 rounded-full shadow-lg hover:bg-blue-700 transition-all duration-300 transform hover:scale-110 z-40">
    <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 3h2l.4 2M7 13h10l4-8H5.4M7 13L5.4 5M7 13l-2.293 2.293c-.63.63-.184 1.707.707 1.707H17m0 0a2 2 0 100 4 2 2 0 000-4zm-8 2a2 2 0 11-4 0 2 2 0 014 0z"/>
    </svg>
    <span class="absolute -top-2 -right-2 bg-red-500 text-white text-xs font-bold rounded-full h-6 w-6 flex items-center justify-center" id="cart-count">0</span>
</button>
@endsection
